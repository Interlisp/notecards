...
XpraClient.prototype.init_settings = function(container) {
	...
	this.debug_categories = ["keyboard", "mouse"];
...
XpraClient.prototype.send = function() {
	...
	this.debug("network", "sending a", arg0, "packet");
	if (this.protocol) {
		if(arg00.substring(0,4) != "ping") {
			//console.log("**send " + this.dbg_ctr++ + " " + arg00);
			//console.dir(arg0);
		}
...
var fn = ctx.packet_handlers[packet_type];
	if (fn==undefined) {
...
var modifiers = get_event_modifiers(event);
	 var newmods = this.translate_modifiers(modifiers);
	 //console.log("old mods " + JSON.stringify(modifiers));
	 //console.log("new mods " + JSON.stringify(newmods));
	return newmods;
};
...
var pkt;
		setTimeout(function () {
			while (me.key_packets.length>0) {
				pkt = me.key_packets.shift();
				me.send(pkt);
				//console.log("**keyb " + me.dbg_ctr++ + "  " + pkt);
			}
		}, delay);
	}
	if (keyname=="F11") {
...
var geom = win.get_internal_geometry();
var wid = win.wid;
var packet = ["configure-window", wid, geom.x, geom.y, geom.w, geom.h, win.client_properties, 0, state, skip_geometry];
	this.send(packet);
};
